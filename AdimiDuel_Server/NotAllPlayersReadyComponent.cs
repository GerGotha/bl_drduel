...
internal class NotAllPlayersReadyComponent : MissionLogic
{
    ...
    public override void OnMissionTick(float dt)
    {
        ...
        var requestedPlayers = (List<PlayerId>)AdimiHelpers.GetField(customServer, "_requestedPlayers")!;
        var actualRequestedPlayers = requestedPlayers.FindAll(p => actualPlayers.Contains(p));
        AdimiHelpers.SetField(customServer, "_requestedPlayers", actualRequestedPlayers);
        ...
        var customBattlePlayers = (List<PlayerId>)AdimiHelpers.GetField(customServer, "_customBattlePlayers")!;
        var actualCustomBattlePlayers = customBattlePlayers.FindAll(p => actualPlayers.Contains(p));
        AdimiHelpers.SetField(customServer, "_customBattlePlayers", actualCustomBattlePlayers);
